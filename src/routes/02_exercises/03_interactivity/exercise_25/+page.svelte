...
<p>
  ...
  /> dataset. Note that we construct two <CC code="Scatterplot.svelte" /> components
  using a
  <a
    target="_blank"
    class="link-primary"
    ...
    >spread operator</a
  >
  in the then-block of <CC code="Exercise.svelte" />:
</p>
...
<p>
  ...
  >
  provides the functionality that you need to draw these rectangles in the
  <a
    target="_blank"
    rel="noreferrer"
    class="link-primary"
...
<p>To implement brushing and linking, perform these steps:</p>
<ol>
  <li>
    Define two component properties in <CC code="Scatterplot.svelte" />. One
  ...
  <li>
    In <CC code="Exercise.svelte" /> use a <CC code="bind" /> to observe the local
    selections of both plots. Then reactively define the global selection so that
    only points that are selected in both plots are set to <CC
      language="javascript"
      code="true"
  ...
  <li>
    ...
    the values of the local selection such that all data points are set to <CC
      language="javascript"
      code="true"
    /> if there is no active selection and only the points that lie within the rectangle
    are set to <CC language="javascript" code="true" /> when there is an active selection.
  </li>
  <li>
